...
export const BarPlot = ({
  ...
  const margin = { left: `${svgWidth * 0.12}`, right: `${svgWidth * 0.06}`, top: `${svgHeight * 0.12}`, bottom: `${tiltXLabels ? svgHeight * 0.25 : svgHeight * 0.15}` }
  ...
  const colorScale = d3.scaleThreshold().domain(colorBreaks.map(b => b.break).slice(1, colorBreaks.length)).range(colorBreaks.map(c => `rgba(${c.rgba[0]},${c.rgba[1]},${c.rgba[2]},1)`))
  
  const xScale = d3.scaleBand().range([0, chartWidth]).padding(0.1).domain(data.map(d => d.type))
  const yScale = d3.scaleLinear().range([chartHeight, 0]).domain([0, d3.max(data.map(d => d.count))]).nice()
